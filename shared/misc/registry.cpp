...
{
  return RegSetValueExA(hReg, VName, 0, REG_DWORD, (UCHAR*)(&val), 4);
};

LONG rSetValue(HKEY hReg, char *VName, DWORD Type, char *Data, DWORD Len)
{
  return RegSetValueExA(hReg, VName, 0, Type, (const UCHAR*)Data, Len);
};
...
DWORD rGetValue(HKEY hReg, char *name)
{
  DWORD data = 0;
  DWORD i = 4;
  RegQueryValueExA(hReg, name, 0, NULL, (PBYTE)&data, &i);
  return data;
};

DWORD rGetValue(HKEY hReg, char *name, char *Data, DWORD Len)
{
  DWORD i = RegQueryValueExA(hReg, name, 0, NULL, (PBYTE)Data, &Len);
  return i?0:Len;
};
...
{
HKEY hKey = 0;
  RegCreateKeyExA(hReg, Name, 0, NULL, REG_OPTION_NON_VOLATILE,
...
{
HKEY hKey = 0;
  RegOpenKeyExA(hReg, Name, 0, KEY_ALL_ACCESS, &hKey);
  return hKey;
};
...
{
DWORD i;
  if (RegQueryValueExA(hReg, Name, 0, NULL, NULL, &i)){
