...
const byte *HTTPInputSource::openStream()
{
  ...
  HINTERNET ihandle = InternetOpenA("Colorer-take5 library connector", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
  if (ihandle == NULL) throw InputSourceException(StringBuffer("Can't create internet connection"));

  DWORD dw;
  HINTERNET iurl = InternetOpenUrlA(ihandle, baseLocation->getChars(), NULL, 0, INTERNET_FLAG_PRAGMA_NOCACHE|INTERNET_FLAG_NO_UI, (DWORD_PTR)&dw);
