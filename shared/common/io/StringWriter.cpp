...
void StringWriter::init(std::string& str , int encoding, bool useBOM){
  this->str = &str;
  file = NULL;
...
void StringWriter::writeBOM(){
  if (useBOM && Encodings::isMultibyteEncoding(encodingIndex)) write(0xFEFF);
};

StringWriter::StringWriter(std::string& str, int encoding = -1, bool useBOM = true){
  init(str, encoding, useBOM);
};
StringWriter::~StringWriter(){
};
void StringWriter::write(wchar c)
{
  byte buf[8];
